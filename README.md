...
工具会先判断问题是否机器人售后相关；第一层审查不通过时停止下探。硬件风险和是否需要人工介入由第二层判断。第二层分类后先匹配 FAQ；FAQ 不能直接覆盖时，再按分流结果进入第四层项目 Debug。使用 Codex CLI 时，Codex 会在只读项目目录中自行检索代码文件。
...
```text
分类：project_debug
处理：进入第四层项目 Debug；使用 Codex CLI 时由 Codex 在只读项目目录中自行检索相关文件
```
